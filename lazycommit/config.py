...
from lazycommit.constants import ALLOWED_TYPES, SCOPE_RE
...
class Config:
    ...
    model: str = field(
        default_factory=lambda: os.environ.get("LAZYCOMMIT_MODEL", DEFAULT_MODEL)
    )
    ...
    max_diff_chars: int = field(
        default_factory=lambda: _env_int("LAZYCOMMIT_MAX_DIFF_CHARS", 12000)
    )
    timeout: float = field(
        default_factory=lambda: _env_float("LAZYCOMMIT_TIMEOUT", 10.0)
    )
    bulk_threshold: int = field(
        default_factory=lambda: _env_int("LAZYCOMMIT_BULK_THRESHOLD", 50)
    )
    force_ai: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if not self.model or not self.model.strip():
            die("model cannot be empty; check LAZYCOMMIT_MODEL or --model")
